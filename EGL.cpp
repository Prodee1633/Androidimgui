    ...
    while (true) {
        ...
        if (selectedTab == 0 && selectedModule >= 0) {
            ...
            if (selectedModule == 0) {
                ...
                // ================= 开始：新增中英文切换功能 =================
                float availWidthToggle = ImGui::GetContentRegionAvail().x;
                ImGui::Text(isChinese ? "中文语言" : "Chinese Language"); // 显示文本
                ImGui::SameLine(availWidthToggle - 65);                   // 将开关靠右对齐
                // 调用你写好的带有动画的开关，绑定 isChinese 变量
                AnimatedToggle("##language_toggle", &isChinese, 60, 28, langToggleAnim);
                ImGui::Spacing();
                ImGui::Spacing();
                // ================= 结束：新增中英文切换功能 =================
